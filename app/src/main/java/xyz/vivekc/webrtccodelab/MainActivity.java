...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, SignallingClient.SignalingInterface {
    private static final String API_ENDPOINT = "https://global.xirsys.net/_turn/MyFirstApp/";
    ...
    private void getIceServers() {
        Log.w("getIceServerList", "Here is getIceServerList method");
        ...
        Call<TurnServerPojo> call = turnServer.getIceCandidates();
        try {
            call.enqueue(new Callback<TurnServerPojo>() {
                @Override
                public void onResponse(Call<TurnServerPojo> call, Response<TurnServerPojo> response) {
                    TurnServerPojo body = response.body();
                    if (body != null) {
                        iceServers = body.getIceServerList().getIceServers();
                    }
                    for (IceServer iceServer : iceServers) {
                        if (iceServer.getCredential() == null) {
                            PeerConnection.IceServer peerIceServer = PeerConnection.IceServer.builder(iceServer.getUrl()).createIceServer();
                            peerIceServers.add(peerIceServer);
                        } else {
                            PeerConnection.IceServer peerIceServer = PeerConnection.IceServer.builder(iceServer.getUrl())
                                    .setUsername(iceServer.getUsername())
                                    .setPassword(iceServer.getCredential())
                                    .createIceServer();
                            peerIceServers.add(peerIceServer);
                        }
                    }

                    Log.w("onApiResponse", "IceServerList\n" + iceServers.toString());
                }

                @Override
                public void onFailure(Call<TurnServerPojo> call, Throwable t) {
                    Log.w("getIceCandidates", "IceServer response on Failure");
                    t.printStackTrace();
                }
            });
        } catch (Exception e) {
            Log.w("getIceServerException", "get Exception");
        }
    }
    ...
    private void doCall() {
        localPeer.createOffer(new CustomSdpObserver("localCreateOffer") {
            ...
            public void onCreateSuccess(SessionDescription sessionDescription) {
                ...
                Log.w("onCreateSuccess", "SignallingClient emit ");
